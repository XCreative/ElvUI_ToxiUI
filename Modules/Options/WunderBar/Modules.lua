        ...
        group.args["panel" .. slot] = {
            ...
            values = function()
                if (panelName == "MiddlePanel") and (slot == 2) then
                    ...
                    local availableModules = { [getNameSorted("")] = "NONE" }
            ...
            set = function(_, value)
                ...
                for _, mods in pairs(E.db.TXUI.wunderbar.modules) do
                    for i, v in ipairs(mods) do if (v == val) then mods[i] = "" end end
                end
...
function O:WunderBar_Modules()
    ...
    -- Modules Left
    do
        local group = self:AddInlineGroup(options, { name = "Modules - Left" })
    ...
    -- Modules Middle
    do
        local group = self:AddInlineGroup(options, { name = "Modules - Middle" })
    ...
    -- Modules Right
    do
        local group = self:AddInlineGroup(options, { name = "Modules - Right" })
