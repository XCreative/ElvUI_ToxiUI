...
function O:WunderBar_SubModules_Time()
    ...
    tab.textGroup.args.textOffset = ACH:Range("Text Offset", nil, 4, { min = -50, max = 100, step = 1 })
    ...
    tab.infoGroup.args.infoFontSize = ACH:Range("Font Size", nil, 6, { min = 1, max = 100, step = 1 }, nil, nil, nil,
                                                infoTextDisabled)

    tab.infoGroup.args.infoOffset = ACH:Range("Vertical Offset", nil, 7, { min = -50, max = 50, step = 1 }, nil, nil,
                                              nil, infoTextDisabled)

    tab.infoGroup.args.infoTextDisplayed = ACH:MultiSelect("Display Information", nil, 8, {
        ["mail"] = "Mail",
        ["date"] = "Current Date",
        ["ampm"] = "AM/PM"
    }, nil, "fill", function(_, key)
