...
function O:WunderBar_SubModules_Additional_Toggle(group)
    ...
    group["toggles"] = ACH:MultiSelect(" ", nil, 1, function()
        local names = {}
        for _, option in pairs(I.HearthstoneData) do
            if (option.known and (not option.hearthstone) and (not option.class) and (not option.portal) and
                (not option.teleport)) then names[option.id] = option.name end
        end
        return names
    end, nil, nil, function(_, key)
...
function O:WunderBar_SubModules_Hearthstone_Select(group, order, name)
    group[name] = ACH:Select((name == "primaryHS" and "Primary" or "Secondary") .. " Hearthstone", nil, order,
                             function()
        -- For mages
        ...
        for _, option in pairs(I.HearthstoneData) do
            if option.known then
                if (option.teleport) or (option.portal) then
                    names[option.id] = F.StringClass(classNames["MAGE"] .. ": ", "MAGE") .. option.name
...
function O:WunderBar_SubModules_Hearthstone()
    ...
    tab.generalGroup.args.iconFontSize = ACH:Range("Icon Size", nil, 3, { min = 1, max = 100, step = 1 }, nil, nil, nil,
                                                   iconDisabled)
    ...
    tab.cooldownGroup.args.cooldownUseAccent = ACH:Toggle("Accent Color", nil, 2, nil, nil, nil, nil, nil,
                                                          cooldownDisabled)
    ...
    tab.cooldownGroup.args.cooldownFontSize = ACH:Range("Font Size", nil, 5, { min = 1, max = 100, step = 1 }, nil, nil,
                                                        nil, cooldownDisabled)

    tab.cooldownGroup.args.cooldownOffset = ACH:Range("Vertical Offset", nil, 6, { min = 1, max = 100, step = 1 }, nil,
                                                      nil, nil, cooldownDisabled)
