...
function O:WunderBar_SubModules_Currency()
    ...
    tab.displayGroup.args.showBagSpace = ACH:Toggle("Show Free Bag Space", nil, 2)
    ...
    for i, info in ipairs(E.global.datatexts.settings.Currencies.tooltipData) do
        if (not info[2] and info[1]) then
            local Group = ACH:Group(F.Uppercase(info[1]), nil, i)
            Group.inline = true
        ...
        elseif info[3] and info[1] then
            ...
            if (tab.currencyGroup.args[tabName]) then
                tab.currencyGroup.args[tabName].args[tostring(i)] =
                    ACH:Toggle(info[1], nil, i, nil, nil, nil, function()
