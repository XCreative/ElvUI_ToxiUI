...
local DC = TXUI:NewModule("Deconstruct", "AceHook-3.0", "AceEvent-3.0", "AceTimer-3.0")
...
function DC:GetItemIdFromLink(txt)
    ...
    local _, itemID = split(":", isItemString)
...
function DC:ToggleHoverAnimations(play)
    ...
    if (self.hoverButton.Pulse) then if (self.hoverButton.Pulse:IsPlaying()) then self.hoverButton.Pulse:Stop() end end
...
function DC:SetState(active)
    if (active == true) then
        ...
        local color = TXUI.AddOnColorRGBA
        ...
        self.bagButton.Texture:SetGradientAlpha("VERTICAL", color.r, color.g, color.b, 0, color.r, color.g, color.b, 0.8)
    ...
    self.bagButton.ttText2 = format(I.Strings.Deconstruct.Status.Text, (self.active) and
                                        I.Strings.Deconstruct.Status.Active or I.Strings.Deconstruct.Status.Inactive)
end
...
function DC:IsUsable(itemID, proffessionState)
    ...
    if (proffessionState == I.Enum.DeconstructState.DISENCHANT) then
        if (self:IsAllowedState(proffessionState)) then
            ...
            if (DC.excludeList[proffessionState] and DC.excludeList[proffessionState][itemID]) then
                return false
            end
            ...
            if not (itemQuality >= LE_ITEM_QUALITY_UNCOMMON and itemQuality <= LE_ITEM_QUALITY_EPIC) then
                return false
            end

            -- Needs to categoriesd as equipment (check comes from blizzard)
            if not (classID == LE_ITEM_CLASS_WEAPON or classID == LE_ITEM_CLASS_ARMOR or
                (classID == LE_ITEM_CLASS_GEM and subclassID == LE_ITEM_GEM_ARTIFACTRELIC)) then return false end

            -- Don't allow cosmetic stuff
            if (classID == LE_ITEM_CLASS_ARMOR) and (subclassID == LE_ITEM_ARMOR_COSMETIC) then return false end
...
function DC:IsEligibleItemFrame(itemFrame)
    ...
    if (professionState == I.Enum.DeconstructState.NONE) and (self:IsUsable(itemId, I.Enum.DeconstructState.DISENCHANT)) then
        professionState = I.Enum.DeconstructState.DISENCHANT
    end

    if (professionState == I.Enum.DeconstructState.NONE) and (self:IsUsable(itemId, I.Enum.DeconstructState.PROSPECT)) then
        professionState = I.Enum.DeconstructState.PROSPECT
    end

    if (professionState == I.Enum.DeconstructState.NONE) and (self:IsUsable(itemId, I.Enum.DeconstructState.MILL)) then
        professionState = I.Enum.DeconstructState.MILL
    end
    ...
    for _, bagFrame in pairs(B.BagFrames) do
        for _, bagID in ipairs(bagFrame.BagIDs) do
            for slotID = 1, GetContainerNumSlots(bagID) do
                ...
                if (not enabled) or (self.db.highlightMode == "NONE") or
                    (self:IsEligibleItemFrame(itemFrame) ~= I.Enum.DeconstructState.NONE) then
...
function DC:CalculateHoverButtonState(itemFrame)
    ...
    if (profesionMode ~= I.Enum.DeconstructState.NONE) then
        ...
        if (spellProffession) then
            ...
            if (not color) then return self:DebugPrint("Could not find colorMap entry", profesionMode) end

            -- Set Label if enabled
            if (self.db.labelEnabled) then
                local label = I.Strings.Deconstruct.Label[profesionMode]
                if (not label) then
                    return self:DebugPrint("Could not find labelMap entry", profesionMode)
                end
            ...
            self.hoverButton.Texture:SetGradientAlpha("VERTICAL", color.r, color.g, color.b, 0, color.r, color.g,
                                                      color.b, 0.8)
...
function DC:OnLeaveHandler()
    if (self.bagID ~= nil) and (self.slotID ~= nil) then
        ...
        if (InCombatLockdown()) then
            self:RegisterEvent("PLAYER_REGEN_ENABLED", "OnCombatEndHandler")
            return
        end
...
function DC:OnCombatEndHandler()
    self:UnregisterEvent("PLAYER_REGEN_ENABLED", "OnCombatEndHandler")
    self.hoverButton:OnLeave()
end

function DC:CreateElements()
    local hoverButton = self.hoverButton or
                            CreateFrame("Button", "TXDeconstructHoverButton", E.UIParent, "SecureActionButtonTemplate")
    ...
    hoverButton.Texture:SetGradientAlpha("VERTICAL", 0, 0, 0, 0, 0, 0, 0, 0)
    ...
    bagButton:Size(16 + E.Border, 16 + E.Border)
    bagButton:SetTemplate()
    bagButton:Point("RIGHT", B.BagFrame.bagsButton, "LEFT", -5, 0)
    ...
    bagButton.Texture:SetGradientAlpha("VERTICAL", 0, 0, 0, 0, 0, 0, 0, 0)
...
function DC:Enable()
    ...
    self:SecureHookScript(_G.GameTooltip, "OnTooltipSetItem", "OnHoverHandler")
    ...
    self:RegisterEvent("SKILL_LINES_CHANGED", "UpdateAllowedStates")
    self:UpdateAllowedStates()
end

function DC:ProfileUpdate()
    self:Disable()

    self.db = E.db.TXUI.addons.deconstruct

    if TXUI:HasRequirements(I.Requirements.Deconstruct) and (self.db and self.db.enabled) then
...
function DC:PLAYER_ENTERING_WORLD()
...
function DC:PLAYER_REGEN_ENABLED()
    self:ProfileUpdate()
end

function DC:Initialize(worldInit)
    -- Set db
    self.db = E.db.TXUI.addons.deconstruct
    ...
    if (not TXUI:HasRequirements(I.Requirements.Deconstruct)) then return end
...
TXUI:RegisterModule(DC:GetName())
