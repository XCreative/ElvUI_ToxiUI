...
local function skinIcon(region, data)
    ...
    region.UpdateInnerOuterSize = function()
        ...
        if (region.outer) then
            region.outer:SetSize(iconWidth + iconBorder + (0.1 * width), iconHeight + iconBorder + (0.1 * height))
        end
    end
    ...
    region.Color = function(_, r, g, b, a)
        ...
        region.icon:SetVertexColor(region.color_anim_r or r, region.color_anim_g or g, region.color_anim_b or b,
                                   region.color_anim_a or a)
    ...
    for _, subRegion in ipairs(region.subRegions) do
        if (subRegion.type == "subglow") and (subRegion.SetGlowType) and (not subRegion.TXSetGlowTypeOld) then
            ...
            subRegion.SetGlowType = function(_, newType)
                subRegion.TXSetGlowTypeOld(subRegion, newType)
                if (newType == "buttonOverlay") then
                    local iconBorder = E:Scale(2)
                    region:AnchorSubRegion(subRegion, "area", nil, nil, iconBorder, iconBorder)
                end
            end
...
function S:WeakAuras()
    ...
    --[[ ICONS ]] --
    if E.db.TXUI.addons.weakAurasIcons.enabled and TXUI:HasRequirements(I.Requirements.WeakAurasIcons) then

        -- Get Shape setting
        pIconMult = (E.db.TXUI.addons.weakAurasIcons.iconShape == I.Enum.IconShape.RECTANGLE) and 0.75 or 1
        ...
        for _, regions in pairs(_G.WeakAuras.regions) do
            if regions.regionType == "icon" then
                skinIcon(regions.region, _G.WeakAuras.GetData(regions.region))
            end
        end
    end

    --[[ BARS ]] --
    if E.db.TXUI.addons.weakAurasBars.enabled and TXUI:HasRequirements(I.Requirements.WeakAurasBars) then
