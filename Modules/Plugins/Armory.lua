...
A.characterSlots = {
    ["HeadSlot"] = { id = 1, needsEnchant = false, direction = A.enumDirection.LEFT },
    ["NeckSlot"] = { id = 2, needsEnchant = false, direction = A.enumDirection.LEFT },
    ["ShoulderSlot"] = { id = 3, needsEnchant = false, direction = A.enumDirection.LEFT },
    ["BackSlot"] = { id = 15, needsEnchant = true, direction = A.enumDirection.LEFT },
    ["ChestSlot"] = { id = 5, needsEnchant = true, direction = A.enumDirection.LEFT },
    ["ShirtSlot"] = { id = 4, needsEnchant = false, direction = A.enumDirection.LEFT },
    ["TabardSlot"] = { id = 18, needsEnchant = false, direction = A.enumDirection.LEFT },
    ["WristSlot"] = {
        id = 9,
        needsEnchant = true,
        enchantCondition = { level = 60, primary = LE_UNIT_STAT_INTELLECT },
        direction = A.enumDirection.LEFT
    },
    ["HandsSlot"] = {
        id = 10,
        needsEnchant = true,
        enchantCondition = { level = 60, primary = LE_UNIT_STAT_STRENGTH },
        direction = A.enumDirection.RIGHT
    },
    ["WaistSlot"] = { id = 6, needsEnchant = false, direction = A.enumDirection.RIGHT },
    ["LegsSlot"] = { id = 7, needsEnchant = false, direction = A.enumDirection.RIGHT },
    ["FeetSlot"] = {
        id = 8,
        needsEnchant = true,
        enchantCondition = { level = 60, primary = LE_UNIT_STAT_AGILITY },
        direction = A.enumDirection.RIGHT
    },
    ["Finger0Slot"] = { id = 11, needsEnchant = true, direction = A.enumDirection.RIGHT },
    ["Finger1Slot"] = { id = 12, needsEnchant = true, direction = A.enumDirection.RIGHT },
    ["Trinket0Slot"] = { id = 13, needsEnchant = false, direction = A.enumDirection.RIGHT },
    ["Trinket1Slot"] = { id = 14, needsEnchant = false, direction = A.enumDirection.RIGHT },
    ["MainHandSlot"] = { id = 16, needsEnchant = true, direction = A.enumDirection.RIGHT },
    ["SecondaryHandSlot"] = {
        id = 17,
        needsEnchant = true,
        enchantCondition = { itemType = LE_ITEM_CLASS_WEAPON },
        direction = A.enumDirection.LEFT
    }
}
...
function A:GetSlotNameByID(slotId)
    for slot, options in pairs(self.characterSlots) do if (options.id == slotId) then return slot end end
end
...
function A:SetupGrowAnimation(obj, hold)
    ...
    if (obj.GrowIn) then
        if (hold and hold > 0) then
            obj.GrowIn.Hold:SetDuration(((hold * holdDuration) * self.db.animationsMult) +
                                            ((fadeDuration * 0.3) * self.db.animationsMult))
        else
...
function A:UpdateItemLevel()
    ...
    if (self.db.stats.showAvgItemLevel) then
        itemLevelText = string.format(string.format("%s / %s", self.db.stats.itemLevelFormat,
                                                    self.db.stats.itemLevelFormat), displayItemLevel, avgItemLevel)
    else
        itemLevelText = string.format(self.db.stats.itemLevelFormat, displayItemLevel)
    end
    ...
    if (self:UseFontGradient(self.db.stats, "itemLevel")) then
        ...
        if (epicComplete) then
            self.frame.ItemLevelText:SetText(F.FastTextGradient(itemLevelText, 0.78, 0.13, 0.57, 0.42, 0.08, 0.82))
        else
            local rareComplete = select(13, GetAchievementInfo(14796))

            if (rareComplete) then
                self.frame.ItemLevelText:SetText(F.FastTextGradient(itemLevelText, 0.01, 0.78, 0.98, 0, 0.38, 0.90))
            else
                self.frame.ItemLevelText:SetText(F.FastTextGradient(itemLevelText, 0.07, 0.90, 0.15, 0, 0.69, 0.11))
            end
        end
    else
...
function A:UpdateTitle()
    ...
    if (self:UseFontGradient(self.db, "nameText")) then
        self.nameText:SetText(F.FastTextGradient(E.myname, 0, 0.6, 1, 0, 0.9, 1))
    else
    ...
    if (self:UseFontGradient(self.db, "titleText")) then
        self.titleText:SetText(F.FastTextGradient(titleName, 0, 0.9, 1, 0, 0.6, 1))
    else
    ...
    local fontIcon = P.wunderbar.subModules.SpecSwitch.icons[primaryTalentTreeIdx] and
                         P.wunderbar.subModules.SpecSwitch.icons[primaryTalentTreeIdx] or
                         P.wunderbar.subModules.SpecSwitch.icons[0]

    if (self:UseFontGradient(self.db, "specIcon")) then
        self.specIcon:SetText(F.StringRGB(fontIcon, classColorNormal))
    else
    ...
    if (self:UseFontGradient(self.db, "classText")) then
        self.classText:SetText(F.FastTextGradient(classNames[currentClass], classColorNormal.r, classColorNormal.g,
                                                  classColorNormal.b, classColorShift.r, classColorShift.g,
                                                  classColorShift.b))
    else
    ...
    self.levelText:SetPoint("RIGHT", self.specIcon, "LEFT", (-iconPadding + self.db.levelTextOffsetX),
                            self.db.levelTextOffsetY)
    self.levelText:SetJustifyH("LEFT")
    self.levelText:SetJustifyV("BOTTOM")

    self.levelTitleText:ClearAllPoints()
    self.levelTitleText:SetPoint("RIGHT", self.levelText, "LEFT", (-textPadding + self.db.levelTitleTextOffsetX),
                                 self.db.levelTitleTextOffsetY)
    self.levelTitleText:SetJustifyH("LEFT")
    self.levelTitleText:SetJustifyV("BOTTOM")

    self.classText:ClearAllPoints()
    self.classText:SetPoint("LEFT", self.specIcon, "RIGHT", (iconPadding + self.db.classTextOffsetX),
                            self.db.classTextOffsetY)
...
function A:UpdatePageStrings(_, slotId, _, slotItem, slotInfo, which)
    ...
    if (self.db.pageInfo.enchantTextEnabled) and (slotInfo.itemLevelColors) and next(slotInfo.itemLevelColors) then
        if (slotInfo.enchantColors and next(slotInfo.enchantColors)) then
            if (slotInfo.enchantText) and (slotInfo.enchantText ~= "") then
                ...
                if (self.db.pageInfo.abbreviateEnchantText) then
                    text = F.Abbreviate(slotInfo.enchantText)
                end
                if (slotOptions.direction == self.enumDirection.LEFT) then
                    slotItem.enchantText:SetText(F.FastTextGradient(text, 0, 0.69, 0.11, 0.07, 0.90, 0.15))
                elseif (slotOptions.direction == self.enumDirection.RIGHT) then
                    slotItem.enchantText:SetText(F.FastTextGradient(text, 0.07, 0.90, 0.15, 0, 0.69, 0.11))
                end
            end
        elseif (self.db.pageInfo.missingEnchantText) and (slotOptions.needsEnchant) then
            if (not slotOptions.enchantCondition) or (self:CheckEnchantCondition(slotOptions)) then
                slotItem.enchantText:SetText(F.StringError("Missing"))
            else
    ...
    if (slotInfo.itemLevelColors) and next(slotInfo.itemLevelColors) then
        ...
        if (not slotItem.TXGradient) then
            ...
            if (slotOptions.direction == self.enumDirection.LEFT) then
                slotItem.TXGradient:Point("BOTTOMLEFT", slotItem, "BOTTOMRIGHT", -1, -1)
            elseif (slotOptions.direction == self.enumDirection.RIGHT) then
                slotItem.TXGradient:Point("BOTTOMRIGHT", slotItem, "BOTTOMLEFT", 1, -1)
            end
        end
        ...
        slotItem.TXGradient:Size(self.db.pageInfo.itemQualityGradientWidth, self.db.pageInfo.itemQualityGradientHeight)

        -- Update Colors
        if (slotOptions.direction == self.enumDirection.LEFT) then
            slotItem.TXGradient.Texture:SetGradientAlpha("HORIZONTAL", r, g, b,
                                                         self.db.pageInfo.itemQualityGradientStartAlpha, r, g, b,
                                                         self.db.pageInfo.itemQualityGradientEndAlpha)

        elseif (slotOptions.direction == self.enumDirection.RIGHT) then
            slotItem.TXGradient.Texture:SetGradientAlpha("HORIZONTAL", r, g, b,
                                                         self.db.pageInfo.itemQualityGradientEndAlpha, r, g, b,
    ...
    for slot, options in pairs(self.characterSlots) do
        if (options.id ~= 4) and (options.id ~= 18) then
            ...
            if (slotFrame.enchantText) then
                F.SetFontFromDB(self.db.pageInfo, "enchant", slotFrame.enchantText, false)
            end
        end
    end
...
function A:UpdateCategoryHeader(frame, animationSlot)
    ...
    if (useHeaderGradient) then
        frame.Title:SetText(F.FastTextGradient(F.StripStringColor(frame.Title:GetText()), 0, 0.9, 1, 0, 0.6, 1))
    else
        frame.Title:SetText(F.StripStringColor(frame.Title:GetText()))
    ...
    leftDivider:Height(2)
    leftDivider:SetTexture(E.media.blankTex)
    leftDivider:SetVertexColor(1, 1, 1, 1)

    if (useHeaderGradient) then
        leftDivider:SetGradientAlpha("HORIZONTAL", 0, 0.6, 1, 0, 0, 0.9, 1, 1)
    else
        local fontColor = F.GetFontColorFromDB(self.db.stats, "header")
        leftDivider:SetGradientAlpha("HORIZONTAL", fontColor.r, fontColor.g, fontColor.b, 0, fontColor.r, fontColor.g,
    ...
    rightDivider:Height(2)
    rightDivider:SetTexture(E.media.blankTex)
    rightDivider:SetVertexColor(1, 1, 1, 1)
    rightDivider:SetGradientAlpha("HORIZONTAL", 0, 0.9, 1, 1, 0, 0.6, 1, 0)

    if (useHeaderGradient) then
        rightDivider:SetGradientAlpha("HORIZONTAL", 0, 0.9, 1, 1, 0, 0.6, 1, 0)
    else
        local fontColor = F.GetFontColorFromDB(self.db.stats, "header")
        rightDivider:SetGradientAlpha("HORIZONTAL", fontColor.r, fontColor.g, fontColor.b, fontColor.a, fontColor.r,
                                      fontColor.g, fontColor.b, 0)
    end
    ...
    leftDivider:Point("LEFT", frame, "LEFT", 3, 0)
    leftDivider:Point("RIGHT", frame.Title, "LEFT", -3, 0)
    rightDivider:ClearAllPoints()
    rightDivider:Point("RIGHT", frame, "RIGHT", -3, 0)
    rightDivider:Point("LEFT", frame.Title, "RIGHT", 3, 0)
...
function A:UpdateCharacterStat(frame, showGradient)
    ...
    if (frame.Label) then
        ...
        if (self:UseFontGradient(self.db.stats, "label")) then
            frame.Label:SetText(F.FastTextGradient(F.StripStringColor(frame.Label:GetText()), 0, 0.6, 1, 0, 0.9, 1))
        else
            frame.Label:SetText(F.StripStringColor(frame.Label:GetText()))
    ...
    if (showGradient and self.db.stats.alternatingBackgroundEnabled) then
        ...
        frame.TXGradient:Point("LEFT", frame, "CENTER")
        frame.TXGradient:Size(90, frame:GetHeight())
        frame.TXGradient:SetTexture(E.media.blankTex)

        if (self:UseFontGradient(self.db.stats, "label")) then
            frame.TXGradient:SetGradientAlpha("HORIZONTAL", 0, 0.6, 1, 0, 0, 0.9, 1,
                                              self.db.stats.alternatingBackgroundAlpha)
        else
            local fontColor = F.GetFontColorFromDB(self.db.stats, "label")
            frame.TXGradient:SetGradientAlpha("HORIZONTAL", fontColor.r, fontColor.g, fontColor.b, 0, fontColor.r,
                                              fontColor.g, fontColor.b, self.db.stats.alternatingBackgroundAlpha)
        end
...
function A:UpdateAttackSpeed(statFrame, unit)
    ...
    statFrame.tooltip =
        _G.HIGHLIGHT_FONT_COLOR_CODE .. format(_G.PAPERDOLLFRAME_TOOLTIP_FORMAT, _G.ATTACK_SPEED) .. " " .. displaySpeed ..
    ...
    _G.PAPERDOLL_STATCATEGORIES = {
        [1] = {
            ...
            stats = {
                ...
                [4] = { stat = "STAMINA" },
                [5] = { stat = "HEALTH", roles = { "TANK" } }, -- Added
                [6] = { stat = "POWER", roles = { "HEALER" } }, -- Added
                [7] = { stat = "ARMOR", roles = { "TANK" } }, -- Modified Smart
                [8] = { stat = "STAGGER", hideAt = 0, roles = { "TANK" }, classes = { "MONK" } }, -- Modified Smart
                [9] = { stat = "MANAREGEN", roles = { "HEALER" } },
                [10] = {
                    stat = "ENERGY_REGEN",
                    hideAt = 0,
                    roles = { "TANK", "DAMAGER" },
                ...
                [11] = { stat = "RUNE_REGEN", hideAt = 0, classes = { "DEATHKNIGHT" } }, -- Added
                [12] = { stat = "FOCUS_REGEN", hideAt = 0, classes = { "HUNTER" } }, -- Added
                [13] = { stat = "MOVESPEED", hideAt = 0 } -- Added
            }
        },
        [2] = {
            ...
            stats = {
                ...
                { stat = "SPELLPOWER", hideAt = 0, primary = LE_UNIT_STAT_INTELLECT, roles = { "HEALER", "DAMAGER" } }, -- Added
                { stat = "CRITCHANCE", hideAt = 0 }, -- 1
                { stat = "HASTE", hideAt = 0 }, -- 2
                { stat = "MASTERY", hideAt = 0 }, -- 3
                { stat = "VERSATILITY", hideAt = 0 }, -- 4
                { stat = "LIFESTEAL", hideAt = 0 }, -- 5
                { stat = "AVOIDANCE", hideAt = 0 }, -- 6
                { stat = "SPEED", hideAt = 0 }, -- 7
                { stat = "DODGE", roles = { "TANK" } }, -- 8
                { stat = "PARRY", hideAt = 0, roles = { "TANK" } }, -- 9
                { stat = "BLOCK", hideAt = 0, showFunc = C_PaperDollInfo.OffhandHasShield } -- 10
            }
        }
    }
end
...
function A:KillBlizzard()
    ...
    for _, frame in ipairs(killList) do if (_G[frame]) then _G[frame]:Kill() end end
...
function A:Enable()
    ...
    self:SecureHook(m, "UpdateCharacterItemLevel", "UpdateItemLevel")
    self:SecureHook(m, "UpdateAverageString", "UpdateItemLevel")
    self:SecureHook(m, "UpdatePageStrings", "UpdatePageStrings")
    self:SecureHook(m, "CreateSlotStrings", "UpdatePageInfo")
    self:SecureHook(m, "UpdateInspectPageFonts", "UpdatePageInfo")
    self:SecureHook(m, "ToggleItemLevelInfo", "ElvOptionsCheck")
    self:SecureHook(_G, "PaperDollFrame_UpdateStats", "UpdateCharacterStats")

    -- Register Events
    self:RegisterEvent("UNIT_NAME_UPDATE", "HandleEvent")
    self:RegisterEvent("PLAYER_PVP_RANK_CHANGED", "HandleEvent")
    self:RegisterEvent("PLAYER_AVG_ITEM_LEVEL_UPDATE", "HandleEvent")
    self:RegisterEvent("PLAYER_TALENT_UPDATE", "HandleEvent")
...
function A:ProfileUpdate()
    ...
    if TXUI:HasRequirements(I.Requirements.Armory) and (self.db and self.db.enabled) then
...
function A:Initialize(worldInit)
    -- Set db
    self.db = E.db.TXUI.armory
    ...
    if (not TXUI:HasRequirements(I.Requirements.Armory)) then return end
