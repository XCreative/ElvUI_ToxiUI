...
function VL:UpdateElements()
    ...
    self.volumeIcon:Point("RIGHT", self.volumeText, "LEFT", -5, 0)
...
function VL:CreateText()
    ...
    volumeText:Point("CENTER")
    volumeIcon:Point("CENTER")
    ...
    self.dataTextDummy.text.SetText = function(_, text)
        ...
        elseif (self.db.textColor == "ACCENT") then
            ...
            displayText = F.StripStringColor(text)
        end
        self.volumeText:SetText(self.db.useUppercase and F.Uppercase(displayText) or displayText)
    end
end
