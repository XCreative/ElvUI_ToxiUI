...
function CR:OnEvent(event)
    ...
    if (event == "ELVUI_FORCE_UPDATE") or (event == "PLAYER_MONEY") or (event == "SEND_MAIL_MONEY_CHANGED") or
        (event == "SEND_MAIL_COD_CHANGED") or (event == "PLAYER_TRADE_MONEY") or (event == "TRADE_MONEY_CHANGED") then
        ...
        if self.db.showBagSpace then
            for i = 0, NUM_BAG_SLOTS do freeBagSpace = freeBagSpace + GetContainerNumFreeSlots(i) end
        end
...
function CR:OnEnter()
    ...
    if self.goldProfit < self.goldSpent then
        DT.tooltip:AddDoubleLine("Deficit:", E:FormatMoney(self.goldProfit - self.goldSpent, style, true), 1, 0, 0, 1,
                                 1, 1)
    elseif (self.goldProfit - self.goldSpent) > 0 then
        DT.tooltip:AddDoubleLine("Profit:", E:FormatMoney(self.goldProfit - self.goldSpent, style, true), 0, 1, 0, 1, 1,
                                 1)
    end
    DT.tooltip:AddLine(" ")
    ...
    for realm in pairs(_G.ElvDB.serverID[E.serverID]) do
        for k, _ in pairs(_G.ElvDB.gold[realm]) do
            if _G.ElvDB.gold[realm][k] then
                local color = E:ClassColor(_G.ElvDB.class[realm][k]) or { r = 1, g = 1, b = 1 }
                tinsert(goldTable, {
                    name = k,
                    realm = realm,
    ...
    for _, g in ipairs(goldTable) do
        ...
        local toonName = format("%s%s%s", nameLine, g.name,
                                (g.realm and g.realm ~= E.myrealm and " - " .. g.realm) or "")
        DT.tooltip:AddDoubleLine((g.name == E.myname and toonName .. " |TInterface/COMMON/Indicator-Green:14|t") or
                                     toonName, g.amountText, g.r, g.g, g.b, 1, 1, 1)
    end
    ...
    if totalAlliance > 0 and totalHorde > 0 then
        if totalAlliance ~= 0 then
            DT.tooltip:AddDoubleLine("Alliance: ", E:FormatMoney(totalAlliance, style, true), 0, .376, 1, 1, 1, 1)
        end
        if totalHorde ~= 0 then
            DT.tooltip:AddDoubleLine("Horde: ", E:FormatMoney(totalHorde, style, true), 1, .2, .2, 1, 1, 1)
        end
        DT.tooltip:AddLine(" ")
    end
    ...
    for _, info in ipairs(E.global.datatexts.settings.Currencies.tooltipData) do
        ...
        if currencyIndex and self.db.enabledCurrencies[currencyIndex] then
            ...
            if currencyInfo then
                ...
                -- Get Icon Texture
                local iconTexture = currencyInfo.iconFileID and
                                        format("|T%s:16:16:0:0:64:64:4:60:4:60|t", currencyInfo.iconFileID) or
                                        format("|T%s:16:16:0:0:64:64:4:60:4:60|t", "136012")
                ...
                if currencyInfo.maxQuantity and currencyInfo.maxQuantity > 0 then
                    rightLine = format("%s / %s", BreakUpLargeNumbers(currencyInfo.quantity),
                                       BreakUpLargeNumbers(currencyInfo.maxQuantity))
                else
    ...
    DT.tooltip:AddDoubleLine("WoW Token:", E:FormatMoney(C_WowTokenPublic_GetCurrentMarketPrice() or 0, style, true), 0,
                             .8, 1, 1, 1, 1)
...
function CR:UpdateText()
    ...
    if self.db.displayedCurrency == "GOLD" then
        ...
        if gold > 0 then
            currencyString = format("%s%s%s", BreakUpLargeNumbers(gold), "g",
                                    (self.db.showSmall and (silver > 0 or copper > 0)) and " " or "")
        end

        if self.db.showSmall then
            if silver > 0 then
                currencyString = format("%s%d%s%s", currencyString, silver, "s", copper > 0 and " " or "")
            end
            if copper > 0 or self.totalGold == 0 then
                currencyString = format("%s%d%s", currencyString, copper, "c")
            end
        end
    else
        local currencyInfo = C_CurrencyInfo_GetCurrencyInfo(currencyIndex)
...
function CR:UpdateElements()
    ...
    self.currencyIcon:Point("RIGHT", self.currencyText, "LEFT", -5, 3)
...
function CR:CreateText()
    ...
    currencyText:Point("CENTER")
    currencyIcon:Point("CENTER")
...
function CR:OnInit()
    ...
    -- Create Token Ticker
    self.tokenTicker = C_Timer_NewTicker(60, C_WowTokenPublic_UpdateMarketPrice)
...
WB:RegisterSubModule(CR, {
    "PLAYER_MONEY", "SEND_MAIL_MONEY_CHANGED", "SEND_MAIL_COD_CHANGED", "PLAYER_TRADE_MONEY", "TRADE_MONEY_CHANGED",
