...
MM.microMenu = {
    ["ach"] = {
        name = ACHIEVEMENTS,
        macro = { LeftButton = _G.SLASH_ACHIEVEMENTUI1 },
    ...
    ["journal"] = {
        ...
        tooltips = {
            MM.leftButtonText .. BINDING_NAME_TOGGLEENCOUNTERJOURNAL, MM.rightButtonText .. "Show Weekly Rewards"
        }
    },
    ["menu"] = {
    ...
    ["lfg"] = {
        ...
        macro = { LeftButton = "/click LFDMicroButton" },
    ...
    ["guild"] = {
        ...
        info = true,
        macro = { LeftButton = "/click GuildMicroButton" },
    ...
    ["spell"] = {
        ...
        macro = { LeftButton = "/click SpellbookMicroButton" },
    ...
    ["talent"] = {
        name = TALENTS_BUTTON,
        macro = { LeftButton = "/click TalentMicroButton" },
    ...
    ["quest"] = {
        ...
        macro = { LeftButton = "/click QuestLogMicroButton" },
    ...
    ["help"] = {
        name = HELP_BUTTON,
        macro = { LeftButton = "/click HelpMicroButton" },
        keyBind = false,
    ...
    ["shop"] = {
        name = BLIZZARD_STORE,
        macro = { LeftButton = "/click StoreMicroButton" },
        keyBind = false,
        newbieTooltip = false,
...
MM.microMenuOrder = {
    "menu", "chat", "guild", "social", "char", "spell", "talent", "ach", "quest", "lfg", "journal", "pvp", "pet",
    "shop", "help", "txui"
}
    ...
    for i, _ in ipairs(self.frames) do
        ...
        if settings.enabled then
            ...
            if frame.id == "txui" and (not CL:HasSeenChangelog()) then
                WB:StartColorFlash(frame.icon, 1, WB:GetFontIconColor(), TXUI.AddOnColorRGBA)
            else
...
function MM:ToxiUITooltip(button)
    ...
    do
        ...
        for _, addOn in ipairs({ "ElvUI", "Details", "Plater", "BigWigs", "WeakAuras", "DBM-Core" }) do
            if F.IsAddOnEnabled(addOn) then
                local data = {}
                data.name = F.StripString(GetAddOnMetadata(addOn, "Title")) or addOn
    ...
    if (not CL:HasSeenChangelog()) then
        DT.tooltip:AddLine(" ")
        DT.tooltip:AddLine(F.StringToxiUI("New Update Installed!"))
    ...
    if button.info.tooltips and self.db.general.additionalTooltips then
        ...
        if (not skipTitle) then
            ...
            if button.info.keyBind then
                DT.tooltip:AddLine(FormatBindingKeyIntoText(button.info.name, button.info.keyBind, "%s %s",
                                                            "|cffffffff(%s)|r"))
            else
...
function MM:ButtonLeave(button)
    if button.id == "txui" and (not CL:HasSeenChangelog()) then
        WB:StartColorFlash(button.icon, 1, WB:GetFontColorFromDB(self.db.general, "icon"), TXUI.AddOnColorRGBA)
    else
...
function MM:PositionButtons()
    ...
    for i, _ in ipairs(self.frames) do
        if self.frames[i].info.special then
            currentWidth = currentWidth + self.db.general.iconFontSize + self.db.general.iconSpacing
        end
    end
...
function MM:CheckGameInfo(gameAccountInfo)
    ...
    if (gameAccountInfo.clientProgram ~= _G.BNET_CLIENT_WOW) then return false end
    if (self.db.general.onlyFriendsWoWRetail and gameAccountInfo.wowProjectID ~= _G.WOW_PROJECT_MAINLINE) then
        return false
    end
    return true
end
...
function MM:OnEvent(event)
    -- Friend counter
    if (self.db.general.infoEnabled and self.db.icons.social.enabled) and
        (event == "ELVUI_FORCE_UPDATE" or event == "BN_FRIEND_INFO_CHANGED" or event == "BN_FRIEND_ACCOUNT_ONLINE" or
            event == "BN_FRIEND_ACCOUNT_OFFLINE" or event == "FRIENDLIST_UPDATE" or event == "CHAT_MSG_SYSTEM") then
                ...
                if (accountInfo) then
                    ...
                    if (numGameAccounts) and (numGameAccounts > 0) then
                        for j = 1, numGameAccounts do
                            if (self:CheckGameInfo(C_BattleNet_GetFriendGameAccountInfo(i, j))) then
                                number = number + 1
                            end
                        end
    ...
    if (self.db.general.infoEnabled and self.db.icons.guild.enabled) and
        (event == "ELVUI_FORCE_UPDATE" or event == "GUILD_ROSTER_UPDATE" or event == "PLAYER_GUILD_UPDATE" or event ==
            "GUILD_MOTD" or event == "CHAT_MSG_SYSTEM") then
...
WB:RegisterSubModule(MM, {
    "GUILD_ROSTER_UPDATE", "PLAYER_GUILD_UPDATE", "GUILD_MOTD", "BN_FRIEND_ACCOUNT_ONLINE", "BN_FRIEND_ACCOUNT_OFFLINE",
    "BN_FRIEND_INFO_CHANGED", "FRIENDLIST_UPDATE", "CHAT_MSG_SYSTEM", "MODIFIER_STATE_CHANGED"
})
