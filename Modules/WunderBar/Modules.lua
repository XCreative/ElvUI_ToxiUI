...
function WB:GrantExtendRequest(module)
    ...
    if self:HasRegisteredSubModule(neighboursNeighbourModule) and neighboursNeighbourModule.extendModuleRequest and
        neighboursNeighbourModule.parentName == "MiddlePanel" then return end
...
function WB:GetModulePoint(module, xOffsetOverwrite)
    ...
    local point, relativePoint, xOffset, yOffset = "LEFT", "LEFT", xOffsetOverwrite and xOffsetOverwrite or
                                                       (width * (module.parentIndex - 1)), 0
...
function WB:UpdatePanelModules(noUpdate)
    ...
    if (not noUpdate) then
        ...
        for _, module in ipairs(self.moduleFrames) do self:GrantExtendRequest(module) end

        -- Request size calculation if dynamic size is enabled
        if self.db.general.experimentalDynamicSize then
            for _, module in ipairs(self.moduleFrames) do
                if module.subModule and module.subModule.OnSizeCalculation then
                    module.calculatedSize = module.subModule:OnSizeCalculation()
                end
            end
            ...
            for _, module in ipairs(self.moduleFrames) do
                ...
                if module.parentIndex == 3 and dynamicSizeFound then
        ...
        for _, module in ipairs(self.moduleFrames) do self:ExtendModule(module) end

        -- Ask Module to update
        for _, module in ipairs(self.moduleFrames) do
            if (module.subModule) then
                if (module.subModule.OnWunderBarUpdate) then module.subModule:OnWunderBarUpdate() end
...
function WB:UpdatePanelSubModules()
    ...
    for _, module in ipairs(self.moduleFrames) do
        ...
        if (moduleData) then
            ...
            subModule.SubModuleHolder = subModule.SubModuleHolder and subModule.SubModuleHolder or
                                            CreateFrame("Frame", nil, module)
            ...
            if subModule.OnEvent then
                if moduleData.events then
                    for _, event in pairs(moduleData.events) do
                        subModule:RegisterEvent(event, "OnEvent")
                    end
                end
            end
...
function WB:DisableAllModules()
    for _, module in ipairs(self.moduleFrames) do
        ...
        if module.subModule then
            module.subModule:UnregisterAllEvents()
...
function WB:ConstructModules()
    ...
    -- Create Modules
    for i = 1, 3 do self:ConstructModule(self.bar.leftPanel, i, i) end
    for i = 4, 6 do self:ConstructModule(self.bar.middlePanel, i, i - 3) end
    for i = 7, 9 do self:ConstructModule(self.bar.rightPanel, i, i - 6) end
