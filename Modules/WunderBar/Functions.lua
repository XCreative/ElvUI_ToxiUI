...
function WB:GetElvUIDataText(name)
    ...
    if dt and (dt.category == nil or dt.category ~= "Data Broker") then
        return dt
    else
        self:DebugPrint("GetElvUIDataText > DT not Found: " .. name)
    end
end

function WB:FlashFontOnEvent(fs, icon)
    if not self.db.general.animationsEvents then return end
    self:StartColorFlash(fs, 0.33 * self.db.general.animationsMult,
                         icon and self:GetFontIconColor() or self:GetFontNormalColor(), self:GetFontAccentColor(), true)
end
...
function WB:SetFontColor(fs, fontColor, animate --[[ = true ]] )
...
function WB:SetColorFunc(obj, objColor, multi, colorType, animate --[[ = true ]] )
    local newColor
...
function WB:SetFontColorFromDB(db, prefix, fs, animate --[[ = true ]] )
    if (animate ~= false) and (self.db.general.animations) then
...
function WB:SetIconFromDB(db, prefix, fs, color --[[ = true ]] , accent)
    local useDB = (db and prefix) and true or false
    ...
    if useDB then
        F.SetFontFromDB({
            ...
            [prefix .. "FontShadow"] = (useDB and db[prefix .. "FontShadow"] ~= nil) and db[prefix .. "FontShadow"] or
                self.db.general.normalFontShadow
        }, prefix, fs, false)
    else
        F.SetFontFromDB(self.db.general, "normal", fs, false)
    end
...
function WB:SetFontFromDB(db, prefix, fs, color --[[ = true ]] , accent, fontOverwrite)
    local useDB = (db and prefix) and true or false
    ...
    if useDB then
        ...
        F.SetFontFromDB(self.db.general, "normal", fs, false, fontOverwrite)
    end
...
function WB:GetElvUIDummy()
    return { text = { SetText = E.noop, SetFormattedText = E.noop } }
end
...
WB.ToxiUIShown = function()
    if E.db.TXUI.wunderbar.general.enabled and TXUI:HasRequirements(I.Requirements.WunderBar) then
        for _, mods in pairs(E.db.TXUI.wunderbar.modules) do
            for slot = 1, 3 do if mods[slot] == "MicroMenu" then return true end end
        end
    end
