...
function WB:StopAnimations(obj)
    if not obj then return self:DebugPrint("StopAnimations > Called with an empty obj") end
...
function WB:StopAllAnimations()
    for animation in pairs(self.registeredAnimations) do self:StopAnimations(animation) end
end
...
function WB:StartTextTransition(obj, duration, callback, noLoop)
    if not obj.TextTransition then
        ...
        obj.TextTransition.Hold:SetDuration(0)
...
function WB:IsTextTransitionPlaying(obj)
    if not obj then return self:DebugPrint("IsTextTransitionPlaying > obj was not defined") end
