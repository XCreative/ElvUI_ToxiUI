...
function ACH:Range(name, desc, order, values, width, get, set, disabled, hidden)
    ...
    for key, value in pairs(values) do optionTable[key] = value end
...
local function SharedMediaSelect(dialogtype, name, desc, order, values, width, get, set, disabled, hidden)
    return {
        type = "select",
        dialogControl = dialogtype,
        name = name,
        desc = desc,
        order = order,
        values = values,
