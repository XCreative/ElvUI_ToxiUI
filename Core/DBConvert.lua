...
function TXUI:DBConvert()
    ...
    if (db.changelog.lastDBConversion ~= nil and db.changelog.lastDBConversion ~= 0) and
        (not cl:IsNewer(TXUI.ReleaseVersion, db.changelog.lastDBConversion)) then return end
    ...
    if db.skins ~= nil then
        ...
        self:DebugPrint("DBConvert > Converted 'Skins'-Profile to new format")
    ...
    if private and private.misc then
        ...
        self:DebugPrint("DBConvert > Converted 'Misc'-Private to new format")

        private.misc = nil
    end

    -- Print debug message
    self:DebugPrint("DBConvert > DB Upgrade finished")
