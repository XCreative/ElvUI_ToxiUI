...
local OnUpdate = function(self, elapsed)
    ...
    if (#self == 0) then self:SetScript("OnUpdate", nil) end
end
...
local AnimMethods = {
    All = {
        ...
        SetOrder = function(self, order)
            ...
            if (order > self.Group.MaxOrder) then self.Group.MaxOrder = order end
        end,
        ...
        SetScript = function(self, handler, func)
            ...
            if Callbacks[handler] then Callbacks[handler][self] = func end
        end,

        GetScript = function(self, handler)
            handler = lower(handler)

            if (Callbacks[handler] and Callbacks[handler][self]) then return Callbacks[handler][self] end
        end,

        Callback = function(self, handler)
            handler = lower(handler)

            if Callbacks[handler][self] then Callbacks[handler][self](self) end
        end,

        StartUpdating = function(self)
            tinsert(Updater, self)

            if (not Updater:GetScript("OnUpdate")) then Updater:SetScript("OnUpdate", OnUpdate) end
        end
    },
    ...
    move = {
        ...
        Reset = function(self)
            ...
            if self.IsRounded then self.ModTimer = 0 end
        end,
...
local GroupMethods = {
    Play = function(self)
        for i = 1, #self.Animations do
            if (self.Animations[i].Order == self.Order) then self.Animations[i]:Play() end
        end
    ...
    Pause = function(self)
        for i = 1, #self.Animations do
            if (self.Animations[i].Order == self.Order) then self.Animations[i]:Pause() end
        end
    ...
    Stop = function(self)
        for i = 1, #self.Animations do self.Animations[i]:Stop() end
    ...
    Finish = function(self)
        for i = 1, #self.Animations do self.Animations[i]:Finish() end
        self:Stop()
    end,
    ...
    CheckOrder = function(self)
        ...
        for i = 1, #self.Animations do
            if (self.Animations[i].Order == self.Order) then
                ...
                if (not self.Animations[i].Playing) then NumDoneAtOrder = NumDoneAtOrder + 1 end
            end
        end
    ...
    CreateAnimation = function(self, style)
        ...
        if (not Initialize[style]) then return end

        local Animation = {}

        -- General methods
        for key, func in pairs(AnimMethods.All) do Animation[key] = func end

        -- Animation specific methods
        if AnimMethods[style] then for key, func in pairs(AnimMethods[style]) do Animation[key] = func end end
...
function TXUI:CreateAnimationGroup(parent)
    local Group = { Animations = {} }

    -- Add methods to the group
    for key, func in pairs(GroupMethods) do Group[key] = func end
...
Update["move"] = function(self, elapsed, i)
    ...
    if (self.Timer >= self.Duration) then
        ...
        self.Parent:SetPoint(self.A1, self.P, self.A2, (self.EndX ~= 0 and self.XOffset or self.StartX),
                             (self.EndY ~= 0 and self.YOffset or self.StartY))
    end
end
...
Update["color"] = function(self, elapsed, i)
    ...
    if (self.Timer >= self.Duration) then
        ...
        Set[self.ColorType](self.Parent,
                            GetColor(self.Timer / self.Duration, self.StartR, self.StartG, self.StartB, self.StartA,
                                     self.EndR, self.EndG, self.EndB, self.EndA))
    end
end
...
Initialize["number"] = function(self)
    ...
    if (not self.StartNumber) then self.StartNumber = tonumber(self.Parent:GetText()) or 0 end
...
Update["frames"] = function(self, elapsed, i)
    ...
    if (self.Timer >= self.Duration) then
        ...
        if (self.Throttle > self.Delay) then
            ...
            while (self.Frame + Advance > self.NumFrames) do self.Frame = self.Frame - self.NumFrames end
